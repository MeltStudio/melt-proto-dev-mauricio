          ...
          {(isCreate || isEdit) ? (
            ...
            >
              {assigneeOptions.map(assignee => (
                <Select.Item key={assignee.name} value={assignee.name}>
                  {assignee.name}
                </Select.Item>
              ))}
            </Select>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                {form.assignee.avatar ? (
                  <img src={form.assignee.avatar} alt={form.assignee.name} className="w-8 h-8 rounded-full" />
                ) : (
                  <div className="w-8 h-8 rounded-full bg-neutral-200 flex items-center justify-center text-default-font">
                    {form.assignee.name?.charAt(0) || "?"}
                  </div>
                )}
